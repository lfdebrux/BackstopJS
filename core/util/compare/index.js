  ...
  return new Promise(function (resolve, reject) {
    ...
    worker.on('message', function (data) {
      ...
      if (data.status === 'fail') {
        ...
        logger.error('ERROR { requireSameDimensions: ' + (data.requireSameDimensions === false ? 'false' : 'true') + ', size: ' + (data.isSameDimensions ? 'ok' : 'isDifferent') + ', content: ' + data.diff.misMatchPercentage + '%, threshold: ' + pair.misMatchThreshold + '% }: ' + pair.label + ' ' + pair.fileName);
      } else {
